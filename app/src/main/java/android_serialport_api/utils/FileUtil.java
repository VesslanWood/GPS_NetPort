...
public class FileUtil {
    ...
    private static boolean deleteDir(File dir, long days) {
        ...
        if (files != null) {
            ...
            for (int i = 0; i < files.length; i++) {
                long createMillTime = getFileCreateTime(files[i].getAbsolutePath());
     ...
     * 保存raw本地文件到Sdcard
     *
     * @param destDir  导出的路径文件夹
     * @param rawId    资源Id
     * @param destName 导出的文件名
     **/
    public static void saveA2SDCard(Context context, String destDir, String destName, int rawId) {
        InputStream inStream = context.getResources().openRawResource(rawId);
        File file = new File(destDir, destName);
        createOrExistsFile(file);
        FileOutputStream fileOutputStream = null;
        ByteArrayOutputStream outStream = null;
        try {
            fileOutputStream = new FileOutputStream(file);//存入SDCard
        ...
        if (file.isDirectory()) {
            ...
            for (File f : files) {//xxxx_2019-09-05_2325
                try {
                    ...
                    LogUtil.e(TAG, "deleteLogFileMyself:" + e.getMessage());
                    try {
                        f.delete();
                    } catch (Exception e1) {
                        e1.printStackTrace();
                        LogUtil.d(TAG, "deleteLogFileMyself,Exception:" + e1.getMessage());
                    }
                    //e.printStackTrace();
                }
            }
            ...
            for (File f : files) {//xxxx_2019-09-05_2325
                for (int i = 0; i < days.size(); i++) {
                    try {
                        ...
                        e.printStackTrace();
                        LogUtil.e(TAG, "getLogFileAcrossDay Exception:" + e.getMessage());
                    }
